...
const confirmNewPhoto = ref(false);
...
async function pickPhoto(event) {
    ...
    confirmNewPhoto.value = true;
}

function saveNewPhoto() {
    confirmNewPhoto.value = false;
    save();
}

function discardNewPhoto() {
    confirmNewPhoto.value = false;
    clearNewPhoto();
}
...
// confirming the removal saves the meal right away, together with any other edits
function removePhotoAndSave() {
    clearNewPhoto();
...
<template>
    ...
    <FitLayout title="Editează masa" :subtitle="dateLabel" :back="`/today?date=${meal.date}`" hide-nav>
        ...
        <BottomSheet :open="confirmNewPhoto" title="Salvezi poza?" @close="discardNewPhoto">
            <img v-if="newPhotoPreview" :src="newPhotoPreview" alt="" class="mb-4 max-h-56 w-full rounded-2xl object-cover"/>
            <p class="text-sm text-white/65">
                {{ meal.photoUrl ? 'Poza nouă o înlocuiește pe cea veche, iar masa se salvează imediat.' : 'Poza se adaugă la masă, iar masa se salvează imediat.' }}
            </p>
            <div class="mt-4 grid grid-cols-2 gap-2">
                <button type="button"
                        class="h-14 rounded-2xl bg-white/5 text-base font-bold text-white active:scale-[0.98]"
                        @click="discardNewPhoto">
                    Renunță
                </button>
        ...
        <BottomSheet :open="confirmRemove" title="Ștergi poza?" @close="confirmRemove = false">
            ...
            <p class="text-sm text-white/65">Poza se șterge definitiv și masa se salvează imediat.</p>
            <div class="mt-4 grid grid-cols-2 gap-2">
                <button type="button"
                        class="h-14 rounded-2xl bg-white/5 text-base font-bold text-white active:scale-[0.98]"
